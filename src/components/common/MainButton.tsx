...
const MainButton = forwardRef<HTMLButtonElement, MainButtonProps>(
  (
    ...
    const variant_hover =
      variant === "primary" ? "hover:bg-white" : "hover:bg-secondary";

    return !isLoading ? (
      <Button
        form={form}
        className={`${isSecondaryVariant ? " text-white  bg-secondary" : "bg-[#FFF9E5]"
          } text-black font-bold  ${propWidth} md:${propWidth}  select-none rounded-[0px] hover:opacity-90 ${variant_hover} ${size_height} ${classes}`}
